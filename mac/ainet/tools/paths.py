"""Path sandboxing under the database root (POSIX (macOS))."""
...
def _assert_component_ok(part: str) -> None:
    ...
    if stem in _RESERVED_WIN:
        raise PathError(f"Reserved name not allowed: {part}")
    if part.endswith(" ") or part.endswith("."):
        raise PathError(f"Name cannot end with space or dot: {part!r}")
    if any(ch in _BAD_CHARS or ord(ch) < 32 for ch in part):
        raise PathError(f"Name contains characters illegal in paths: {part!r}")
...
class DbPaths:
    ...
    def resolve(self, relative: str | Path, *, must_exist: bool = False) -> Path:
        ...
        # Build with PurePosixPath parts so path separators stay correct.
