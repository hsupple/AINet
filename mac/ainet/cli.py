...
def _configure_stdio() -> None:
    """Prefer UTF-8 on macOS Terminal so JSON output handles unicode cleanly."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="ainet", description="AINet database tools (macOS)")
    parser.add_argument(
        "--db",
        type=Path,
    ...
    call.add_argument(
        ...
        nargs="?",
        default="{}",
        help='JSON object of arguments. On zsh you can pass JSON inline, or use --args-file or --args-stdin.',
    )
    call.add_argument(
        "--args-file",
        type=Path,
        help="Read JSON arguments from a UTF-8 file (handy for long JSON).",
    )
    call.add_argument(
        "--args-stdin",
        action="store_true",
